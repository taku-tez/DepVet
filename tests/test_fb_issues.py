...
class TestVersionContextInMonitor:
    ...
    def test_version_context_passed_to_deep_analyze_in_monitor(self):
        ...
        monitor_section = src[src.find("async def _monitor") :]
        assert "version_context" in monitor_section, "_process_one must pass version_context= to deep.analyze()"
    ...
    <package>
      ...
      <versionInfo>8.1.0</versionInfo>
        ...
        try:
            ...
            assert "requests" in names or len(entries) > 0, f"SPDX XML should parse packages, got: {entries}"
        finally:
    ...
    <component type="library">
      ...
      <version>5.3.0</version>
        ...
        try:
            ...
            assert len(entries) > 0, f"CycloneDX XML without PURL should use name fallback, got: {entries}"
